...
namespace LaneDetector{
    	...
    	void IPMworldToPixels(LaneDetectorConf &laneDetectorConf, const cv::Point2d &inPoint, cv::Point2d &outPoint)
    	{
		double m = laneDetectorConf.m;
		double n = laneDetectorConf.n;
		double h = laneDetectorConf.h;
...
//    }//end IPMgetPixelCoordinates
    	...
    	void DrawMarkingFromIPM(cv::Mat &ipmMat,
                           ...
                           LaneDetectorConf &laneDetectorConf)
    	{
        	...
        	{
            		//! left lane marking
            		cv::circle(ipmMat, leftPoints[j+1], 3, CV_RGB(255, 255, 0));
            		cv::line(ipmMat, leftPoints[j], leftPoints[j+1], CV_RGB(255, 0, 0), 2);

            		//! right lane marking
            		cv::circle(ipmMat, rightPoints[j+1], 3, CV_RGB(255, 0, 255));
            		cv::line(ipmMat, rightPoints[j], rightPoints[j+1], CV_RGB(0, 255, 0), 2);
        	}
    	}
